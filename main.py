...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "7751803155:AAF92lei1_APL0CIEU5sxGXn49KqmljC25c")
...
def generate_new_voice_audio(text, voice=None, style=None):
    ...
    try:
        ...
        url = "https://sii3.top/api/voice.php"   # ✅ عدلت الرابط
        params = {
            "text": text,
            "voice": voice
        }
...
def fetch_voices_list():
    """Fetch available voices from the external API (filter female by default)."""
    try:
        url = "https://sii3.top/api/voice.php"   # ✅ عدلت الرابط
...
def fetch_voices_list():
    """Fetch available voices from the external API."""
    try:
        url = "https://sii3.moayman.top/api/voices.php"
        response = requests.get(url, timeout=30)
        if response.status_code == 200:
...
def handle_voice_selection(call):
    """Handle voice selection by admin (step 1: choose voice)."""
    user_id = call.from_user.id
    if not is_admin(user_id):
...
def handle_style_selection(call):
    """Handle style selection after choosing voice (step 2)."""
    user_id = call.from_user.id
    if not is_admin(user_id):
        bot.answer_callback_query(call.id, "⛔️ هذه الميزة للمشرفين فقط.")
        return
...
def handle_download(call):
    ...
    try:
        ...
        if result.get("status") == "ok":
            ...
            audio_data.seek(0)
            audio_data.name = f"{title}.mp3"   # مهم عشان يظهر كـ mp3
...
def handle_edit_description(message):
    ...
    if error:
        ...
        media_group = []
        for i, img_url in enumerate(images[:4]):  # نرسل max 4 صور
